...
At startup the runtime requires hardware build ID `0xA833`, checks that the
HWH records a 160 MHz RFDC output/fabric clock, and independently measures that
clock in the FPGA against the 100 MHz PS clock. It also measures the final
...
stops before an FFT if either the deployed clock or final stream is wrong.
...
| A 0.5 MHz offset is displayed as about 4 MHz | Hardware build ID must be `0xA833`; HWH and measured fabric clock must both be 160 MHz; output TVALID interval must be 16 |
