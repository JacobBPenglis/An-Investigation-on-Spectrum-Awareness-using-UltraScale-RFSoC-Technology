...
At startup the runtime requires hardware build ID `0xA832` and directly
measures the final AXI-stream TVALID interval. The required interval is 16
cycles of the 160 MHz RFDC fabric clock (10 MSPS). An interval of 128 is the
deepest PL decimation path (1.25 MSPS) and causes the observed factor-of-eight
FFT frequency-axis error.
...
| A 0.5 MHz offset is displayed as about 4 MHz | Hardware build ID must be `0xA832`; output TVALID interval must be 16, not 128 |
